...
% D = jensen_shannon_divergence(bins,bins);
% D=kullback_leibler_divergence(bins,bins);
% D = pdist2(bins,bins,'mahalanobis');
...
step = sqrt(P*hatD)\L; %% instead of IP=inv(sqrt(P*hatD));
L2 = step/sqrt(P*hatD); %% instead of L2=IP*L*IP;

%   L2 = normalizeMatrix(L2,4); %%%%%%%% maybe a normalization step would
%   be useful


%%% Solve for g's in equ. 2
[uu,~,~]=svd(L2);
g = sqrt(P*hatD)\uu; %% instead of g=IP*uu;
