...
numU = size(V(index_te),1);  % number of test instances
% prediction 
predscore = V(index_te, :) * model.W + repmat(model.bias, numU, 1);
%%=============if you want to run the prediction function of liblinear

% for i = 1:size(testLabels,2)
% [predicted_label, accuracy, decision_values(:,i)] = predict(testLabels(:,i), sparse(V(index_te, :)), model(i));
% end
